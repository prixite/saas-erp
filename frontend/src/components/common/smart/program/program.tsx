...
import ProgramModal from "@src/components/shared/popUps/programModal/programModal";
...
import "@src/components/common/smart/program/program.scss";

function Program() {
  const { data: rows = [], isLoading } = useApiProgramsListQuery();
  const [deleteProgram] = useApiProgramsDestroyMutation();

  const [action, setAction] = useState("add");
  const [dataLoading, setIsDataLoading] = useState(true);
  const [rowCellId, setRowCellId] = useState<number | undefined>(undefined);
  const [programData, setProgramData] = useState<ProgramInterface[]>([]);
  ...
  return (
    <Box className="programsDataGridTable-section">
      <Box
        className="top-bar-cls"
        sx={{ display: "flex", justifyContent: "space-between" }}
      >
        <Typography className="title-cls">{"Programs"}</Typography>
        <Box
      ...
      {!dataLoading ? (
        ...
        <>
          <RowSkeletonCard pathString="program" />
        </>
      )}
      <ProgramModal
        programId={rowCellId}
      ...
      <DeleteModal
        ...
        handleObjDelete={handleProgramDelete}
