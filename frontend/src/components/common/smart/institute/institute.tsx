...
import InstituteModal from "@src/components/shared/popUps/instituteModal/instituteModal";
import { timeOut } from "@src/helpers/constants/constants";
import { InstitueInterface } from "@src/helpers/interfaces/localizationinterfaces";
import { toastAPIError } from "@src/helpers/utils/utils";
import {
  useApiInstituesListQuery,
  useApiInstituesDestroyMutation,
} from "@src/store/api";
import "@src/components/common/smart/institute/institute.scss";

function Institute() {
  const { data: rows = [], isLoading } = useApiInstituesListQuery();
  const [deleteInstitute] = useApiInstituesDestroyMutation();

  const [action, setAction] = useState("add");
  const [dataLoading, setIsDataLoading] = useState(true);
  const [rowCellId, setRowCellId] = useState<number | undefined>(undefined);
  const [instituteData, setInstituteData] = useState<InstitueInterface[]>([]);
  ...
  const handleInstitueDelete = async () => {
    await deleteInstitute({
      id: rowCellId,
    })
      .unwrap()
      .then(async () => {
        toast.success("Institure deleted successfully", {
          autoClose: timeOut,
          pauseOnHover: false,
        });
  ...
  return (
    <Box className="instituteDataGridTable-section">
      <Box
        className="top-bar-cls"
        sx={{ display: "flex", justifyContent: "space-between" }}
      >
        <Typography className="title-cls">{"Institutes"}</Typography>
        <Box
      ...
      {!dataLoading ? (
        ...
        <>
          <RowSkeletonCard pathString="institute" />
        </>
      )}
      <InstituteModal
        institueId={rowCellId}
        action={action}
        open={openModal}
      ...
      <DeleteModal
        ...
        handleObjDelete={handleInstitueDelete}
